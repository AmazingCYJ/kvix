...
const (
	// StandardFIO 基于标准文件系统调用（os.File）的 IO 管理器实现。
	// 支持完整的读写操作，适用于常规数据读写路径。
	StandardFIO FileIOType = iota
	// MemoryMap 基于内存映射文件（mmap）的 IO 管理器实现。
	// 提供高性能的读操作，适用于启动阶段或读多写少的只读场景。
	MemoryMap
)
...
type IOManager interface {
	// ReadAt 从指定偏移读取数据到 p 中，不改变当前文件读写位置。
	// 返回实际读取的字节数和可能的错误（包括 io.EOF）。
	ReadAt(p []byte, off int64) (n int, err error)
	// Write 从当前写入位置追加写入数据，通常用于顺序追加写。
	// 返回实际写入的字节数和可能的错误。
	Write(p []byte) (n int, err error)
	// Sync 强制把缓冲区数据刷新到磁盘，确保持久化。
	// 在需要保证数据安全性的场景下调用。
	Sync() error
	// Close 关闭底层资源（文件描述符或 mmap 映射）。
	Close() error
	// Size 返回文件当前大小（单位：字节），供恢复和边界判断使用。
	Size() (int64, error)
}

// NewIOManager 根据配置选择具体的 IO 实现并返回对应的 IOManager。
// 1. StandardFIO：标准文件 IO，适合常规读写路径。
// 2. MemoryMap：mmap IO，适合启动阶段或读多写少的只读场景。
